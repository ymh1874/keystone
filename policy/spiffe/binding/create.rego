# METADATA
# description: Policy for creating SPIFFE bindings
package identity.spiffe.binding.create
...
# The `input.target.binding` is an enriched SPIFFE binding object:
...
#     domain: {     domain-scoped authorization (tagged union variant)
...
#     }
#     project: {    project-scoped authorization (tagged union variant)
#       project_id:      string        project ID
#       project:         object|null   resolved Project; null if not found or error
#       role_ids:        [string]      role IDs
#       roles:           [object]      resolved RoleRefs; missing roles skipped
#     }
#     system: {     system-scoped authorization (tagged union variant)
...
#   ]
#
# The `input.existing` is null for create operations.

default allow := false

# Admin (admin role) can create bindings.
allow if {
...
# System users (system == "all") with member role can create bindings.
allow if {
	"member" in input.credentials.roles
	input.credentials.system == "all"
	not input.target.binding.is_system
	not spiffe_common.authorization_domains_missing
	not spiffe_common.authorization_projects_missing
	not spiffe_common.authorization_roles_missing
}

# Owner can create their own bindings.
allow if {
...
violation contains {"field": "domain_id", "msg": "creating SPIFFE binding for other domain requires `admin` role."} if {
	spiffe_common.foreign_binding
	not "admin" in input.credentials.roles
	not input.credentials.is_admin
}

violation contains {"field": "role", "msg": "creating SPIFFE binding requires `manager` role."} if {
	spiffe_common.own_binding
	not "member" in input.credentials.roles
}

violation contains {"field": "is_system", "msg": "creating system-wide SPIFFE binding requires `admin` role."} if {
	input.target.binding.is_system == true
	not "admin" in input.credentials.roles
	not input.credentials.is_admin
}
